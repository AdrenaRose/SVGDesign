...
function renderTriangle(triangle) {
  ...
  <polygon points="200,10 300,190 115,190" fill="${triangle.shapeColor}" />
    ...
    <text x="185" y="150" font-size="60" text-anchor="middle" fill="${triangle.textColor}">${triangle.text}</text>
  </svg>`;
}
...
function renderSquare(square) {
  return `<svg
    ...
    xmlns="http://www.w3.org/2000/svg"
  >
    <rect x="50" y="50" width="300" height="300" fill="${square.shapeColor}" />

    <text x="190" y="200" font-size="60" text-anchor="middle" fill="${square.textColor}">${square.text}</text>
  </svg>`;
}
